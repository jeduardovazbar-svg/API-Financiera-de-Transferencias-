const verificarAppToken = (req, res, next) => {
  ...
  // 2. Si el cliente no envi� ning�n token, rechazamos
  if (!tokenRecibido) {
  ...
  if (tokenRecibido !== process.env.APP_TOKEN) {
    return res.status(403).json({ 
      mensaje: "Acceso denegado. El token de aplicacion es inválido." 
    });
  }

  // 4. Si el token es correcto, permitimos que la petici�n contin�e a la ruta
  next();
};
